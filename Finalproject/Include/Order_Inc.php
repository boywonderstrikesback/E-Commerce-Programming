<form action='<? echo $_SERVER['PHP_SELF']; ?>' method='POST' name='order_form' id='order_form' enctype='multipart/form-data'>
	...
	$select_producst = "SELECT bjj_id, category, description, size, keyword, code, price, stock_quantity, photo
		FROM supplements
	...
	<?php
		$select_address = "SELECT address_1, address_2, city, supplement_states_id, zip from supplementt_customers WHERE supplement_customers_id=$supplement_customers_id";
		...
		if(!$exec_select_address){
			...
			$bjj_states_id = $one_record['bjj_states_id'];
			$zip = $one_record['zip'];
		}
		...
		$select_Match = "SELECT bjj_matches_id, Opponent, Win_loss,Method from bjj_states";
		$exec_select_Match = @mysqli_query($link, $select_Match);
		if(!$exec_select_states){
			exit("The following error occurred: ".mysqli_error($link));
			mysqli_close($link);
		}else{
			$multi_array = array();
			while($one_record = mysqli_fetch_assoc($exec_select_Match)){
				$multi_array[] = $one_record;
			}
			create_drop_down_from_query('Opponent: ', 'bjj_Matches_id', 'bjj_matches_id', $multi_array, ['tabindex'=>'9', 'title'=>'Opponent'], $errors_array);
		}
	...
	<fieldset><legend>Shipping Method</legend>	
		<?php
		$select_carriers_methods = "SELECT bjj_carriers_methods_id, carrier, method, fee from supplement_carriers_methods";
		...
		if(!$exec_select_carriers_methods){
			...
			create_drop_down_from_query('Shipping Method: ', 'bjj_carriers_methods_id', 'bjj_carriers_methods_id', $multi_array, ['title'=>'Shipping Method'], $errors_array);
		}
		?>
	</fieldset>
	<fieldset>
