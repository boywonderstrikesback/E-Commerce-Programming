...
if(isset($_POST['bjj_states_id'])){
	$supplement_states_id = $_POST['supplement_states_id'];
}else{
	$errors_array['bjj_states_id'] = "Please pick a state!";
}
...
if(count($errors_array)==0){
	...
	$insert_into_supplement_customers = "INSERT INTO bjj_customers (first_name, last_name, email, phone, password, address_1, address_2, city, bjj_states_id, zip, date_created) VALUES
	('$first_name', '$last_name', '$email', '$phone', '".password_hash($password, PASSWORD_BCRYPT)."', '$address_1', '$address_2', '$city', $bjj_states_id, '$zip', '$date_created')";
	$exec_insert_into_bjj_customers = @mysqli_query($link, $insert_into_bjj_customers);
	if(!$exec_insert_into_bjj_customers){
		rollback("The following error occurred when inserting into bjj_customers: ".mysqli_error($link));
	}else{
