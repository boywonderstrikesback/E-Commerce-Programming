<form action='<? echo $_SERVER['PHP_SELF']; ?>' method='POST' name='account_update_form' id='account_update_form' enctype='multipart/form-data'>
	<fieldset><legend>Registration</legend>
		<?php
			...
			$select_bjj = "SELECT bjj_matches_id,Opponent,Win_Loss,Method from bjj_states";
			$exec_select_bjj = @mysqli_query($link, $select_bjj);
			if(!$exec_select_bjj){
				exit("The following error occurred: ".mysqli_error($link));
				mysqli_close($link);
			}else{
				$multi_array = array();
				while($one_record = mysqli_fetch_assoc($exec_select_bjj)){
	...
	<p>
		<label>
			<input type='hidden' value='<?php echo $supplement_customers_id; ?>' name=supplement_customers_id' id='form_submitted' />
